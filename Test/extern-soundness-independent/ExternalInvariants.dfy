...
module ExternalInvariants {
  ...
  twostate lemma IndependentValidityInductive(v: ExternallyValid, changed: set<object>)
      requires old(AllValid(set v': ExternallyValid | allocated(v') && v'.P()))
      requires forall o :: o !in changed ==> unchanged(o)
      requires forall v': ExternallyValid :: v' in changed && v' in v.ValidatableRepr ==> v'.Valid()
